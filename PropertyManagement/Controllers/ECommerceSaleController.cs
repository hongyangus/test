...
{
    [Authorize]
    public class ECommerceSaleController : BaseController
    {
        private string reporttitle = "E Commerce Sales"; // Specify the report title here
        ...
        //                try
        //                {
        //                    cmd.CommandText = "insert into tblEbayOrder(ordernumber, sku, title, quantity, cost, sales, ebayFee, paypalFee, shippingFee,internationalShippingFee, date, profit, pieces, discount, name, state, storeId) "
        //                        + " values ("
        //                        + "" + order.ebay_id + ","
        //                        + "'" + order.sku + "',"
        //                        + "'" + order.title + "',"
        ...
        [AllowAnonymous]
        [HttpPost]
        public PartialViewResult ChartView(int comparisonVariable, int valueType, int chartType, int drillDownLevel, string[] vendors, string[] skus, string[] accounts, string[] warehouses, string[] states, string startDate, string endDate, bool dispalyNegative)
        {
